...
void Button::render(bool mouseDown, bool mouseUp, bool buttonInteraction) {
    ...
    textPos.x += (this->buttonWidth * 0.5) * 0.5 - (textSize.x / 2.0);
    
    //Vertically center the text so its center is at the button's center
    textPos.y += (this->buttonHeight * 0.5) * 0.5 - (textSize.y / 2.0);
    
    GLfloat sizeFraction = textSize.x / (this->buttonWidth * 0.5); //This represents what fraction of the button the text will take up
...
void Button::updateMouse(bool mouseDown, bool mouseUp) {
    ...
    actualButtonX *= framebufferSize.x;
    actualButtonY *= framebufferSize.y;
    actualButtonWidth *= framebufferSize.x;
    actualButtonHeight *= framebufferSize.y;
